...
cv.createTrackbar('gamma','gamma_correction',0,300,nothing)

cv.setTrackbarPos('gamma','gamma_correction', 100)

roi = gImg('picture2','jpg')
...
while True:
    ...
    g = cv.getTrackbarPos('gamma','gamma_correction') / 100

    for i in range(roi.shape[1]):
        for j in range(roi.shape[0]):
            output.itemset(j,i,255 * (roi_gray.item(j,i)/255)**g)
