...
roi = cv.imread('C:\\Users\\PresentJay\\Desktop\\ai_ml_vision\\AI-ImageProcessing-Training\\img\\{}.{}'.format('OpenClose-binary-test','bmp'))
...
for i in range(1,roi_gray.shape[0]-1):
    for j in range(1,roi_gray.shape[1]-1):
        if roi_gray.item(i,j) == 0:
            if roi_gray.item(i-1,j) == 255 or roi_gray.item(i+1,j) == 255 or roi_gray.item(i,j-1) == 255 or roi_gray.item(i,j+1) == 255:
